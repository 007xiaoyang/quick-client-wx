		...
		<view class="flex-center" :style="{ width: `${width}rpx` }">
			<view :class="[isJianDisabled || disabled ? 'opacity-6 gray' : '']">
				<tm-button
					...
					:black="black_tmeme"
					@click="setStep('-')"
					:item-class="circular ? ' pa-0' : ` round-l-${round} `"
					icon-size="24"
					:round="circular ? round : 0"
			...
			<input
				...
				v-model="setVal"
				type="digit"
				:style="{ height: `${height}rpx`, width: `calc(100% - ${height}rpx)` }"
			...
			<view :class="[isAddDisabled || disabled ? 'opacity-6 gray' : '']">
				<tm-button
					...
					@longpress="longpressEvent('+')"
					:fllowTheme="fllowTheme"
					:shadow="isAddDisabled || disabled ? 0 : shadwo_num"
					:black="black_tmeme"
					@click="setStep('+')"
					:item-class="circular ? ' pa-0' : ` round-r-${round} `"
					icon-size="24"
					:round="circular ? round : 0"
					:theme="disabled ? '' : color_tmeme"
