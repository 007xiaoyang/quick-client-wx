	...
	return new Promise((resolve, reject) => {
		...
		if (Array.isArray(requestInfo.files)) {
			...
			async function fileUpload(i) {
				...
				// 压缩
				item.path = await compressImage(requestInfo,  item.path);
...
// 压缩图片
export const compressImage = function(requestInfo, path) {
	return new Promise(async(resolve, reject) => {
		if (requestInfo.compress) {
			await uni.compressImage({
				src: path,
				quality: requestInfo.quality || 80,
				success(compressRes) {
